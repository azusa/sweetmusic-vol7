...
Debugビルドでは有効、Releaseビルドでは無効になっています。
...
ビルドスクリプトの処理がApp Centerでのビルド時に行われるようにするには、
...
次に、Androidプロジェクトの`MainActity.cs`内のAppCenterのSDKの起動処理を[@lst:code_050_050]の通りに修正します。
